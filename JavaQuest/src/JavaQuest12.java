...
public class JavaQuest12 {
  ...
  public static int[] twoSum(int[] a , int target){
   ...
   for (int i = 0 ; i < a.length-1; i++){
    // if the element in the array is greater than or equal target , skip the loop
      if(a[i]>= target){
        continue;
      }else {
        for (int j = i+1 ; j < a.length ; j++ ){
          // check if greater then target , if yes skip the loop 
          if(a[j]>= target){
            continue;
          }else {
              if ((a[i] + a[j]) == target) {
